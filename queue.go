...
func (cb *CircularBuffer) Push(work Work) {
	cb.mutex.Lock()
...
func (cb *CircularBuffer) Get(index int) (Work, error) {
	cb.mutex.RLock()
	defer cb.mutex.RUnlock()

	if index < 0 || index >= cb.size {
		return Work{}, errors.New("index out of range")
	}
...
var chunkBuffer = NewCircularBuffer()
